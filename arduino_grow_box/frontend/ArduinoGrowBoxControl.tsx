...
interface ArduinoGrowBoxData {
  temperature_1?: number; temperature_2?: number; temperature_3?: number; temperature_4?: number;
  humidity_1?: number; humidity_2?: number; humidity_3?: number; humidity_4?: number;
  water_level?: number;
}
interface PowerButtonProps { label: string; isOn: boolean; loading: boolean; onToggle: () => void | Promise<void> }

const PowerButton: React.FC<PowerButtonProps> = ({ label, isOn, loading, onToggle }) => (
  <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
    <button
      type="button" onClick={onToggle} disabled={loading}
      style={{
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        width: '82px', height: '82px', borderRadius: '50%', border: '2px solid #0d6efd',
    ...
    >
      <div
        style={{
          position: 'relative', width: '38px', height: '38px', borderRadius: '50%', border: '3px solid #ffffff',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      >
        <div style={{
          ...
          borderRadius: '2px', backgroundColor: '#ffffff',
          boxShadow: isOn ? '0 0 6px rgba(255,255,255,0.9)' : 'none',
          transition: 'box-shadow 0.3s ease-in-out'
        }}/>
        <div style={{
          width: '20px', height: '20px', borderRadius: '50%', borderWidth: '3px', borderStyle: 'solid',
          borderColor: '#ffffff', borderTopColor: 'transparent', opacity: isOn ? 1 : 0.7,
          boxShadow: isOn ? '0 0 8px rgba(255,255,255,0.9)' : 'none',
          transition: 'opacity 0.3s ease-in-out, box-shadow 0.3s ease-in-out'
        }}/>
      </div>
    </button>
    <div style={{ marginTop: '0.5rem', fontSize: '0.85rem', color: isOn ? '#51CBEE' : '#ffffff' }}>
...
const ArduinoGrowBoxControl: React.FC<SensorControlProps> = ({ sensorName }) => {
  ...
  const [targets, setTargets] = useState<{ temp_target_min?: number | null; temp_target_max?: number | null; hum_target_min?: number | null; hum_target_max?: number | null; }>({})
  const [currentValues, setCurrentValues] = useState<{ avg_temperature?: number | null; avg_humidity?: number | null; }>({})
  const [ledStatus, setLedStatus] = useState<{ minutes_on_today?: number | null; minutes_until_on?: number | null; minutes_until_off?: number | null; last_toggle?: string | null; }>({})
  ...
  const fetchFase = useCallback(async () => {
    try {
      const resp = await fetch(`/sensors/arduino-grow-box/${sensorName}/fase`)
      if (resp.ok) {
  ...
  const fetchCultivationStatus = useCallback(async () => {
    try {
      const resp = await fetch(`/sensors/arduino-grow-box/${sensorName}/stato-coltivazione`)
      if (resp.ok) {
        const result = await resp.json()
        setCultivationActive(result.cultivation_active || false)
        if (result.growth_phase) setFase(result.growth_phase)
  ...
  const toggleAction = async (endpoint: string, desiredState: boolean, setState: React.Dispatch<React.SetStateAction<boolean>>) => {
    const key = `${endpoint}-${desiredState}`
    setActionLoading(prev => ({ ...prev, [key]: true }))
    try {
      const resp = await fetch(`/sensors/arduino-grow-box/${sensorName}/${endpoint}?action=${desiredState ? 'on' : 'off'}`, { method: 'POST' })
      if (!resp.ok) {
        const errJson = await resp.json().catch(() => ({}))
        throw new Error(errJson.detail || `Errore ${resp.status}`)
      }
  ...
  const iniziaColtivazione = async () => {
    ...
    try {
      const resp = await fetch(`/sensors/arduino-grow-box/${sensorName}/inizia-coltivazione`, { method: 'POST' })
      if (!resp.ok) {
        const errJson = await resp.json().catch(() => ({}))
        throw new Error(errJson.detail || `Errore ${resp.status}`)
      }
  ...
  const fineColtivazione = async () => {
    ...
    setCultivationLoading(true)
    try {
      const resp = await fetch(`/sensors/arduino-grow-box/${sensorName}/fine-coltivazione`, { method: 'POST' })
      if (!resp.ok) {
        const errJson = await resp.json().catch(() => ({}))
        throw new Error(errJson.detail || `Errore ${resp.status}`)
      }
  ...
  const setFaseHandler = async (nuovaFase: string) => {
    setFaseLoading(true)
    try {
      const resp = await fetch(`/sensors/arduino-grow-box/${sensorName}/fase?fase=${nuovaFase}`, { method: 'POST' })
      if (!resp.ok) {
        const errJson = await resp.json().catch(() => ({}))
        throw new Error(errJson.detail || `Errore ${resp.status}`)
      }
  ...
  if (loading && Object.keys(data).length === 0) {
    return <div style={{ padding: '2rem', textAlign: 'center', color: '#F4B342' }}><p>Caricamento dati...</p></div>
  }
  if (error) {
    return (
      <div style={{ padding: '2rem', textAlign: 'center', color: '#F44336' }}>
        <p>Errore: {error}</p>
        <button type="button" onClick={fetchAllData} style={{
          marginTop: '1rem', padding: '0.5rem 1rem', backgroundColor: '#F4B342',
          color: '#1A1A1A', border: 'none', borderRadius: '4px', cursor: 'pointer'
        }}>Riprova</button>
      </div>
    )
  }
  ...
  const renderTemperatureSensor = (index: number) => {
    ...
    return (
      <div key={`temp-${index}`} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem',
  ...
  const renderHumiditySensor = (index: number) => {
    ...
    return (
      <div key={`hum-${index}`} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem',
        padding: '0.75rem', backgroundColor: 'rgba(0, 0, 0, 0.2)', borderRadius: '8px' }}>
        <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ width: '100%', minHeight: '100vh', padding: '2rem', backgroundColor: '#360185', color: '#FFFFFF' }}>
      ...
      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
        <button type="button" onClick={iniziaColtivazione} disabled={cultivationLoading}
          style={{ padding: '0.75rem 1.25rem', background: '#4CAF50', border: 'none', borderRadius: '6px', color: '#fff', cursor: 'pointer' }}>
          Avvia coltivazione
        </button>
        <button type="button" onClick={fineColtivazione} disabled={cultivationLoading}
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {['piantina', 'vegetativa', 'fioritura'].map(opt => (
            <button key={opt} type="button" disabled={faseLoading} onClick={() => setFaseHandler(opt)}
              style={{
          ...
          <div style={{
            ...
            <div style={{ padding: '1rem', backgroundColor: 'rgba(0, 0, 0, 0.2)', borderRadius: '8px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '1.4rem', fontWeight: 'bold', color: '#4CAF50' }}>
                {tempMin !== null && tempMax !== null ? `${tempMin}°C - ${tempMax}°C` : 'N/A'}
              </div>
            </div>
            <div style={{ padding: '1rem', backgroundColor: 'rgba(0, 0, 0, 0.2)', borderRadius: '8px', textAlign: 'center' }}>
            ...
            <div style={{ padding: '1rem', backgroundColor: 'rgba(0, 0, 0, 0.2)', borderRadius: '8px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '1.4rem', fontWeight: 'bold', color: '#2196F3' }}>
                {humMin !== null && humMax !== null ? `${humMin}% - ${humMax}%` : 'N/A'}
              </div>
            </div>
            <div style={{ padding: '1rem', backgroundColor: 'rgba(0, 0, 0, 0.2)', borderRadius: '8px', textAlign: 'center' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '1rem' }}>
          <PowerButton label="Luce LED" isOn={luceLedOn} loading={!!actionLoading['luce-led']} onToggle={() => toggleAction('luce-led', !luceLedOn, setLuceLedOn)} />
          <PowerButton label="Ventola" isOn={ventolaOn} loading={!!actionLoading['ventola']} onToggle={() => toggleAction('ventola', !ventolaOn, setVentolaOn)} />
          <PowerButton label="Resistenza" isOn={resistenzaOn} loading={!!actionLoading['resistenza']} onToggle={() => toggleAction('resistenza', !resistenzaOn, setResistenzaOn)} />
          <PowerButton label="Pompa Aspirazione" isOn={pompaAspirazioneOn} loading={!!actionLoading['pompa-aspirazione']} onToggle={() => toggleAction('pompa-aspirazione', !pompaAspirazioneOn, setPompaAspirazioneOn)} />
          <PowerButton label="Pompa Acqua" isOn={pompaAcquaOn} loading={!!actionLoading['pompa-acqua']} onToggle={() => toggleAction('pompa-acqua', !pompaAcquaOn, setPompaAcquaOn)} />
        </div>
      </div>
